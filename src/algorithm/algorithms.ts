...
export const quickSort = <T>(arr: T[]): T[] => {
    ...
    const partition = (array: T[], low: number, high: number): number => {
        const pivot = array[high];  // 使用数组最后一个元素作为基准值
        let i = low - 1;
    ...
    const quickSortHelper = (array: T[], low: number, high: number): void => {
        if (low < high) {
